...
from elasticai.hw_measurements.driver import DriverDMM6500, DriverNGUX01
...
class TestHandlerAmplifier:
    ...
    def __init__(
        self, com_sets: DriverPort = DriverPortIES, en_debug: bool = False, only_plot: bool = False
    ) -> None:
        ...
        if not self._en_debug and not only_plot:
            ...
            self._hndl_src.serial_open_known_target(resource_name=com_sets.com_ngu, do_reset=True)
            self._hndl_src.do_beep()
            sleep(1)

            self._hndl_daq = DriverDMM6500()
            self._hndl_daq.serial_start_known_target(resource_name=com_sets.com_dmm, do_reset=True)
    ...
    def run_transfer_test(self, chnnl_num: int) -> dict:
        ...
        results = self._hndl_test.run_test_transfer(
            ...
            func_set_daq=self._hndl_src.set_voltage
            if not self._en_debug
            else self._hndl_test.dummy_set_daq,
            func_sens=self._hndl_src.get_measurement_voltage
            if not self._en_debug
            else self._hndl_test.dummy_get_daq,
...
def run_test() -> None:
    ...
    hndl = TestHandlerAmplifier(en_debug=False, only_plot=bool_only_plot)
