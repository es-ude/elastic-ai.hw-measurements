...
from elasticai.hw_measurements import MetricNoise, TransientData, TransientNoiseSpectrum
...
def extract_noise_metrics(
    ...
    dut.load_data(time=data.timestamps, signal=data.rawdata, channels=data.channels)
    metrics: MetricNoise = dut.extract_transient_metrics()
    dut.extract_noise_power_distribution(scale=scale_adc, num_segments=10000)
    dut.exclude_channels_from_spec(exclude_channels)
    noise: TransientNoiseSpectrum = dut.remove_power_line_noise(tolerance=5.0, num_harmonics=5)
