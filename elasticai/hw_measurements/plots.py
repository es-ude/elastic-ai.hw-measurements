...
from elasticai.hw_measurements import (
    FrequencyResponse,
    TransformSpectrum,
    TransientData,
...
def plot_transfer_function_norm(
    data: dict,
    path2save: str = "",
    xlabel: str = "Stimulus Input",
    ylabel: str = "Stimulus Output",
    ...
    for idx, key in enumerate(data.keys()):
        if not key == "stim":
            plt.step(
                val_input, data[key]["mean"], where="mid", marker=".", c=get_plot_color(idx), label=key
            )
            plt.fill_between(
                val_input,
                data[key]["mean"] - data[key]["std"],
                data[key]["mean"] + data[key]["std"],
                step="mid",
                alpha=0.3,
                color="gray",
...
def plot_transfer_function_metric(
    data: dict,
    func: object,
    path2save: str = "",
    xlabel: str = "Stimulus Input",
    ...
    for key in data.keys():
        if not key == "stim":
            ...
            data_metric.update({key: {"mean": metric, "std": scale_val * data[key]["std"]}})
...
def plot_fra_data(
    ...
    ax1.tick_params(axis="both", labelsize=get_font_size() - 1)
    ...
    ax2.tick_params(axis="y", labelsize=get_font_size() - 1)
...
def plot_spectrum_noise(
    data: TransientNoiseSpectrum, file_name: str = "noise", path2save: str = "", show_plot: bool = False
) -> None:
    ...
    freq_min = np.min(np.array([dat0[0] if not dat0[1] == 0.0 else dat0[1] for dat0 in data.freq]))
