...
from elasticai.hw_measurements import FrequencyResponse, TransformSpectrum, TransientData
from elasticai.hw_measurements._helper.csv import CsvHandler
...
def load_spectral_data(path: Path, file_name: str, begin_line: int = 33) -> TransformSpectrum:
    ...
    """
    data = CsvHandler(path=str(path), file_name=file_name, delimiter=",").read_data_from_csv(
        include_chapter_line=False, start_line=begin_line, type_load=float
    )
    return TransformSpectrum(freq=data[:, 0], spec=data[:, 1], sampling_rate=1.0)
