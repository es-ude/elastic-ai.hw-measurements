...
from torch import Tensor, cat, ones_like, zeros
...
class ExperimentCreator(ExperimentMain):
    ...
    def __init__(self, device_id: int) -> None:
        ...
        yaml_handler = YamlConfigHandler(
            set, yaml_name=f"Config_{device_id:03d}_Creator", start_folder=get_path_to_project()
        )
...
def run_inference_on_target(device_id: int, block_plot: bool = False) -> None:
    ...
    cnt_true = 0
    cnt_false = 0
    for tensor_in, data_get, tensor_exp in zip(
        data_inference_use, data_inference_out, inference_expected
    ):
