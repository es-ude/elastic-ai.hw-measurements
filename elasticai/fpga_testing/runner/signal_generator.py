...
def generate_sinusoidal_waveform(
    f_sig: float, fs: float, no_periods=1, bitwidth=16, signed_out=False
...
def generate_noise(time: np.ndarray, sigma: float, bit: int = 16) -> np.ndarray:
    ...
    if bit > 16:
        ...
        data_type = "int8"
    return np.array(object=np.random.normal(0, sigma, time.size), dtype=data_type)
