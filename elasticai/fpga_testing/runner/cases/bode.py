...
class ExperimentBode(ExperimentMain):
    ...
    def __init__(self, device: type[InterfaceRunner], device_id: int) -> None:
        ...
        yaml_handler = YamlConfigHandler(
            set,
            yaml_name=f"Config_{device_id:03d}_Bode",
    ...
    def generate_sinusoidal_signal(
        self, f_sig: float, no_periods: int = 10, sigma_noise=0.0
        ...
        if sigma_noise > 0.0:
            data_signal += generate_noise(
                time=discrete_time, sigma=sigma_noise, bit=self.__settings_bode.bitwidth_filter
            )
...
def run_filter_on_target(device: type[InterfaceRunner], device_id: int, block_plot: bool = False) -> Path:
    ...
    data_dut = {
        ...
        "gain_dut": [],
        "phase_dut": [],
        "gain_emu": bode_ref["gain"],
        "phase_emu": bode_ref["phase"],
        "f_sig": f_sig,
        "f_smp": settings_bode.sampling_rate,
    }
    ...
    for f_sig_used in f_sig:
        ...
        gain, phase = exp0.extract_gain_phase(f_sig=f_sig_used, xin=data_norm["wvf"], xout=data_out)
...
def plot_bode(f_sig: np.ndarray, data_dut: dict, path: str = "", block_plot: bool = False) -> None:
    ...
    # --- Plot: DUT
    ax0.semilogx(f_sig, gain_dut, marker=".", markersize=4, color=get_plot_color(0), label="DUT")
    ax0.set_ylabel("Gain (dB)")
    ax0.grid(True)

    ax1.semilogx(f_sig, phase_dut, marker=".", markersize=4, color=get_plot_color(0))
    ...
    ax0.semilogx(
        f_sig,
        20 * np.log10(data_dut["gain_emu"]),
        marker=".",
        markersize=4,
        color=get_plot_color(1),
        label="Emulation",
    )
    ax1.semilogx(f_sig, data_dut["phase_emu"], marker=".", markersize=4, color=get_plot_color(1))
