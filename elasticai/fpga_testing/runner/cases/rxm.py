...
class ExperimentRxM(ExperimentMain):
    ...
    def preprocess_ram_data(self, adr_start: int = 0) -> np.ndarray:
        ...
        if self.__settings_rom.adrwidth_rom > self.get_adrwidth_structure:
            raise ValueError(
                f"Selected ADRWIDTH ({self.__settings_rom.adrwidth_rom} vs. {self.get_adrwidth_structure}) is higher than on hardware implemented"
            )
        ...
        self._buffer_data_send = self._device.slice_data_for_transmission(
            data_write_into_mem + data_read_from_mem
        )
        return waveform_ana
...
def plot_call(x_in: np.ndarray, x_out: np.ndarray, path: str = "", block_plot: bool = False) -> None:
    ...
    plt.figure()
    plt.plot(x_in, marker=".", markersize=4, color=get_plot_color(0), label="Write")
    plt.plot(x_out[start_pos:], marker="o", markersize=4, color=get_plot_color(1), label="Read")
