...
from elasticai.fpga_testing._helper.signal_generator import generate_sinusoidal_waveform
...
class SettingsEcho:
    ...
    def get_data_in(self) -> np.ndarray:
        return generate_sinusoidal_waveform(
            f_sig=self.freq_signal,
            fs=self.sampling_rate,
            no_periods=self.num_periods,
            bitwidth=self.bitwidth_data,
            signed_out=self.signed_data,
        )[1]
...
class ExperimentEcho(ExperimentMain):
    ...
    def __init__(self, device: type[InterfaceRunner], device_id: int) -> None:
        ...
        yaml_handler = YamlConfigHandler(
            set,
            yaml_name=f"Config_{device_id:03d}_Echo",
        ...
        self.__data_scaling_value = self._device._get_data_scaling_value(
            self.__settings_echo.bitwidth_data
        )
    ...
    def preprocess_data(self, waveform: np.ndarray) -> None:
        ...
        self._buffer_data_send = self._device.slice_data_for_transmission(
            data=DataProcessor().preparing_data_streaming_architecture(
                signal=waveform,
    ...
    def postprocess_data(self) -> np.ndarray:
        ...
        data_return = self._device.slice_data_from_transmission(
            data=self._buffer_data_get, is_signed=self.__settings_echo.signed_data
        )
        return np.asarray(data_return[:-2:2]) / self.__data_scaling_value


def run_echo_on_target(device: type[InterfaceRunner], device_id: int, block_plot: bool = False) -> Path:
    ...
    """
    # --- Preparing Test
    exp0 = ExperimentEcho(device=device, device_id=device_id)
    settings_echo = exp0.get_settings_func

    # Control Routine
    exp0.init_experiment(f"echo_{device_id:02d}")
    ...
    # Saving results
    data_dut = {"process_time": time_run, "data_in": data_used, "data_out": data_out}
    np.save((exp0.get_path2run / "results_echo.npy").as_posix(), data_dut, allow_pickle=True)
    time0 = (
        np.linspace(0, data_used.shape[0], data_used.shape[0], endpoint=False)
        / settings_echo.sampling_rate
    )
    plot_transient(
        t0=time0,
        xin=data_used,
        xout=data_out,
        path=exp0.get_path2run.as_posix(),
...
def plot_transient(
    ...
    if fsig:
        ...
        plt.plot(
            ...
            marker=".",
            markersize=4,
            color=get_plot_color(1),
            label="Output",
        )
    else:
        plt.plot(t0, xin, marker=".", markersize=4, color=get_plot_color(0), label="Input")
        plt.plot(t0, xout, marker=".", markersize=4, color=get_plot_color(1), label="Output")
