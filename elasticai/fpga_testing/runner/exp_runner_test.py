...
from .exp_runner import ExperimentMain, ExperimentSettings, extract_available_structures_on_device


def test_get_path2run() -> None:
    rslt = ExperimentMain(device=DummyRunner).get_path2run
    check = get_path_to_project("runs")
    assert rslt.as_posix() == check.as_posix()


def test_get_settings() -> None:
    rslt: ExperimentSettings = ExperimentMain(device=DummyRunner).get_settings
...
def test_init_experiment() -> None:
    rslt = ExperimentMain(device=DummyRunner).init_experiment(test_name="test")
...
def test_get_do_inference() -> None:
    dut = ExperimentMain(device=DummyRunner)
