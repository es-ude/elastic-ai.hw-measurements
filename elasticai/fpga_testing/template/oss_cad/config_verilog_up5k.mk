...
.PHONY: synthesize
synthesize: $(BUILD_OUT)/synth/$(TOP).json $(BUILD_OUT)/synth/$(TOP).v

$(BUILD_OUT)/synth/$(TOP).json $(BUILD_OUT)/synth/$(TOP).v:
	mkdir -p $(BUILD_OUT)/synth
	yosys \
	    -qql $(BUILD_OUT)/synth/$(TOP).log \
	    -p 'read_verilog -sv $(VLOG_SRC); hierarchy -top $(TOP); synth_ice40 -top $(TOP); tee -o $(BUILD_OUT)/$(TOP)_util_sync.rpt stat -width; write_json $(BUILD_OUT)/synth/$(TOP).json; write_verilog $(BUILD_OUT)/synth/$(TOP).v;'
